describe("Bundles_Compass", function () {
  it("Gets, types and visual tests", function () {
    ...
    cy.get("[data-qa=button-bundle]").click().wait(2000);
    ...
    // Enable GWP Filter
    cy.get('[data-testid="pilter-tags-Gift With Purchase"]').click().wait(2000);

    //snapshots page for comparison
    cy.matchImageSnapshot("bundPLP_GwpFilter");

    // Disable GWP Filter
    cy.get('[data-testid="pilter-tags-Gift With Purchase-selected"]')
      .click()
      .wait(3000);

    // Enable Pik Filters
    cy.get('[data-testid="pilter-tags-Pik TV"]').click().wait(2000);

    //snapshots page for comparison
    cy.matchImageSnapshot("bundPLP_PikFilter");

    // Disable Pik Filter
    cy.get('[data-testid="pilter-tags-Pik TV-selected"]').click().wait(3000);

    // Enable Optik Filter
    cy.get('[data-testid="pilter-tags-Optik TV"]').click().wait(2000);
